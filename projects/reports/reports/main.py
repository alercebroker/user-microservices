...
async def get_report_list(q: QueryByReport = Depends()):
    return await crud.query_reports(connection, q)
...
async def get_report_list_by_object(q: QueryByObject = Depends()):
    return await crud.query_reports_by_object(connection, q)
...
async def count_reports_by_day(q: QueryByDay = Depends()):
    return await crud.count_by_day(connection, q)
...
async def get_single_report(report_id: str):
    ...
    report = await crud.get_report(connection, report_id)
    if report is None:
